...
public:
	...
	bool Open(const char * conf) {
		...
		if (!m_client)
		{
            qDebug() << "MIDI:coremidi: No client created.";
			return false;
		}
		...
		if (!m_port)
		{
            qDebug()<< "MIDI:coremidi: No port created.";
			return false;
		}
	...
	void PlayMsg(Bit8u * msg) {
        qDebug() << "playmsg" << hex << msg[0] << msg[1] << msg[2] << m_port;
        ...
        qDebug() << "ListAdd" << MIDIPacketListAdd(packetList, (ByteCount)sizeof(packetBuf), m_pCurPacket, (MIDITimeStamp)0, len, msg);
		
		// Send the MIDIPacketList
        qDebug() << "send" << MIDISend(m_port,m_endpoint,packetList);
	}
